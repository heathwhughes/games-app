...
export const createGame: RequestHandler = (req, res, next) => {
    ...
    const loser = (req.body as {loser_id: number}).loser_id;
    const winner_character = (req.body as {winner_character_id: number}).winner_character_id;
    const loser_character = (req.body as {loser_character_id: number}).loser_character_id;
    ...
    (async () => {
        ...
        try {
            ...
            if (winner_points_results.rowCount === 0) {
                await pool.query(
                    `INSERT INTO users_characters (user_id, character_id, points) 
            ...
            await pool.query(
                `INSERT INTO games (winner_user_id, loser_user_id, winner_character_id, loser_character_id, point_value) 
...
export const deleteGame: RequestHandler<{id: string}> = (req, res, next) => {
    // TODO?
}

function determineGamePointValue(winnerPoints: number, loserPoints: number): number {
...
function calculatePointValues(differential: number): [number, number] {
    if (differential <= 50) {
        return [25, 25];
